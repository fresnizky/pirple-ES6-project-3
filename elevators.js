...
class Building {
  ...
  addElevator(minFloor, maxFloor) {
    ...
    if (minFloor < this.minFloor || maxFloor > this.maxFloor) {
      throw `minFloor must be greater than or equal to ${
        this.minFloor
      } and maxFloor must be less than or equal to ${this.maxFloor}`;
    }
    if (minFloor > maxFloor) {
      throw `minFloor must be greater than maxFloor`;
    }
  ...
  addPassenger(from, to) {
    ...
    // Check if passenger is going from minFloor to maxFloor or viceversa
    if (
    ...
    console.log(
      `[LOG] Passenger ${passenger.id} pushed button ${direction} on floor ${
        floor.floor
      }. Destination ${to}`
    );
  ...
  setElevatorDestination(elevator, destination) {
    if (!elevator.isMoving) {
      ...
      if (
        ...
        console.log(
          `[LOG] Updated elevator ${elevator.id} destination from ${
  ...
  moveElevator(elevator) {
    // If elevator is stopped set new directon
    if (!elevator.isMoving) {
      const floor = this.floors.find(element => {
      ...
      if (elevator.passengers.length) {
        // Set new destination based on elevator passengers
        elevator.direction = elevator.passengers[0].direction;
      } else if (floor.passengers.length) {
        // Set new destination based on current floor passengers
        const waitingPassengers = floor.passengers.filter(element => {
          return (
            element.isMultiElevator ||
            (element.to >= elevator.minFloor && element.to <= elevator.maxFloor)
          );
        });
        if (waitingPassengers.length) {
          const direction = waitingPassengers[0].direction;
          waitingPassengers.forEach(element =>
            floor.passengers.splice(
              floor.passengers.findIndex(
                item => item.direction === element.direction
              ),
              1
            )
          );
    ...
    // Check if elevator arrived to its final destination.
    if (elevator.floor === elevator.destination) {
      // Stop the elevator
      // console.log(
      //   `[LOG] Elevator ${elevator.id} stopped on floor ${elevator.floor}`
      // );
    ...
    if (elevator.direction) {
      elevator.floor =
        elevator.direction === 'down' ? elevator.floor - 1 : elevator.floor + 1;
      console.log(
        `[LOG] Elevator ${elevator.id} moving ${
          elevator.direction
        }. Current floor: ${elevator.floor}. Destination: ${
          elevator.destination
        }`
      );
    }
    ...
    // Check if a passenger is entering elevator on this floor
    if (floor.passengers.length) {
      const waitingPassengers = floor.passengers.filter(element => {
    ...
    // Check if a passenger is leaving elevator on this floor
    if (elevator.passengers.length) {
      const travellingPassengers = elevator.passengers.filter(element => {
        return (
          element.to === elevator.floor ||
          (element.isMultiElevator && elevator.floor === 0)
        );
      });
      travellingPassengers.forEach(element =>
        elevator.passengers.splice(
          elevator.passengers.findIndex(item => item.to === elevator.floor),
          1
        )
      );
  ...
  enterElevator(elevator, waitingPassengers) {
    console.log(
      `[LOG] Elevator ${elevator.id} on floor ${
        elevator.floor
      }. Passengers waiting ${waitingPassengers.length}`
    );
    setTimeout(() => {
  ...
  openEnterElevator(elevator, waitingPassengers) {
    ...
    console.log(
      `[LOG] Passenger ${passenger.id} entered elevator ${
        elevator.id
      }. Destination: ${passenger.to}`
    );

    // If passenger is multi elevator set destination to lobby
    this.setElevatorDestination(
      elevator,
      !passenger.isMultiElevator ? passenger.to : 0
    );

    if (waitingPassengers.length) {
      setTimeout(() => {
        this.openEnterElevator(elevator, waitingPassengers);
      }, PASSENGER_ENTER_ELEVATOR);
    } else {
      setTimeout(() => {
  ...
  leaveElevator(elevator, travellingPassengers) {
    console.log(
      `[LOG] Elevator ${elevator.id} on floor ${
        elevator.floor
      }. Passengers leaving elevator ${travellingPassengers.length}`
    );
    setTimeout(() => {
  ...
  openLeaveElevator(elevator, travellingPassengers) {
    ...
    if (!passenger.isMultiElevator) {
      ...
      floor.passengers.push(passenger);
      const selectedElevator = this.selectElevator(
        floor.floor,
        passenger.to,
        passenger.direction
      );
    ...
    console.log(
      `[LOG] Passenger ${passenger.id} left elevator ${elevator.id} on floor ${
        elevator.floor
      }.`
    );
  ...
  printSummary() {
    this.passengers.map(passenger => {
      ...
      console.log(
        `Passenger: ${passenger.id}. From: ${passenger.from}. To: ${
          passenger.to
...
const createRandomPassenger = () => {
  ...
  let to = 0;
  from = Math.floor(
    Math.random() * (myBuilding.maxFloor - myBuilding.minFloor) +
      myBuilding.minFloor
  );
  do {
    to = Math.floor(
